...
PasswordGrantType.prototype.getUser = function(request, client) {
  ...
  // if (!request.body.deviceId || request.body.deviceId == 0) {
  //   throw new InvalidRequestError('Invalid parameter: `deviceId`');
  // }

  return promisify(this.model.getUser, 3)(request.body.username, request.body.password, client)
    .then(function(user) {
      if (!user) {
