...
var TokenHandlerOTP = require('./handlers/token-handler-otp');
...
/**
 * Create a token by OTP.
 */

OAuth2Server.prototype.tokenByOTP = function(request, response, options, callback) {
  options = _.assign({
    accessTokenLifetime: 60 * 60,             // 1 hour.
    refreshTokenLifetime: 60 * 60 * 24 * 14,  // 2 weeks.
    allowExtendedTokenAttributes: false
  ...
  return new TokenHandlerOTP(options)
    .handle(request, response)
    .nodeify(callback);
};
